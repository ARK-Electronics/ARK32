...
void proportionalBrake()
{
	...
	PHASE_A_GPIO_PORT_HIGH->CFGHR &= ~(0xf << 8);
	PHASE_A_GPIO_PORT_HIGH->CFGHR |= (0x3 << 8);
	...
	PHASE_C_GPIO_PORT_HIGH->CFGHR &= ~(0xf << 0);
	PHASE_C_GPIO_PORT_HIGH->CFGHR |= (0x3 << 0);
...
void phaseBPWM()
{
	if (!eepromBuffer.complementary_pwm) { // for future
		...
		PHASE_B_GPIO_PORT_LOW->CFGLR &= ~(0xf << 0);
		PHASE_B_GPIO_PORT_LOW->CFGLR |= (0xb << 0); //low pwm
	}
	PHASE_B_GPIO_PORT_HIGH->CFGHR &= ~(0xf << 4);
	PHASE_B_GPIO_PORT_HIGH->CFGHR |= (0xb << 4); //high pwm
}
void phaseBFLOAT()
{
	PHASE_B_GPIO_PORT_LOW->CFGLR &= ~(0xf << 0);
...
void phaseCPWM()
{
	...
	PHASE_C_GPIO_PORT_HIGH->CFGHR &= ~(0xf << 0);
	PHASE_C_GPIO_PORT_HIGH->CFGHR |= (0xb << 0);
}
...
void phaseAPWM()
{
	if (!eepromBuffer.complementary_pwm) {
		...
		PHASE_A_GPIO_PORT_LOW->CFGLR &= ~(0xf << 4);
		PHASE_A_GPIO_PORT_LOW->CFGLR |= (0xb << 4); //low pwm
	}
	PHASE_A_GPIO_PORT_HIGH->CFGHR &= ~(0xf << 8);
	PHASE_A_GPIO_PORT_HIGH->CFGHR |= (0xb << 8); //high pwm
}
