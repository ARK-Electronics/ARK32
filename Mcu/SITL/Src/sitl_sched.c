...
static void sitl_sem_post(sitl_sem_t *s)
{
...
__attribute__((unused)) static void sitl_sem_wait(sitl_sem_t *s)
{
	WaitForSingleObject(*s, INFINITE);
}
__attribute__((unused)) static bool sitl_sem_wait_2s(sitl_sem_t *s)
{
...
void sitl_fw_loop_tick(void)
{
	...
	__atomic_store_n(&fw_alive_until_ns, sim_time_ns_v + sitl_cfg.sim.fw_lag_max_ns, __ATOMIC_SEQ_CST);
	gate_wake();
}
		...
		if (best == SITL_IRQ_COMP || best == SITL_IRQ_COM) {
			...
			if (lat > 20000 && prints < 12) {
				...
				fprintf(stderr,
					...
					" act=[%u,%u,%u,%u,%u,%u]\n",
					best, lat * 1e-3, sim_time_ns_v * 1e-9, blocked.steps_primask, blocked.steps_disabled,
					blocked.steps_free, blocked.steps_active[0], blocked.steps_active[1], blocked.steps_active[2],
					blocked.steps_active[3], blocked.steps_active[4], blocked.steps_active[5]);
			}
	...
	if (sitl_cfg.nosleep) {
		...
		if (f) {
			...
			if (fscanf(f, "%ld", &v) == 1 && v != -1) {
				fprintf(stderr, "SITL: WARNING: RT throttling is enabled and will stall "
...
static void *sim_thread_main(void *arg)
{
	...
	for (;;) {
		...
		if (primask && current_irq < 0) {
             ...
             */
			if ((irq_pending & irq_enabled) != 0 && __atomic_load_n(&section_grant_ns, __ATOMIC_SEQ_CST) < 2000) {
				...
				if (sect != gave_up_section) {
					...
					while (primask && current_irq < 0 && (irq_pending & irq_enabled) != 0 &&
			...
			if (sitl_cfg.sim.fw_lag_max_ns != 0) {
				...
				while (sim_time_ns_v >= __atomic_load_n(&fw_alive_until_ns, __ATOMIC_SEQ_CST)) {
					...
					if (sitl_cfg.nosleep) {
						...
						while (!(renewed = sim_time_ns_v < __atomic_load_n(&fw_alive_until_ns, __ATOMIC_SEQ_CST)) &&
						       wallclock_ns() - w0 < 10000000) {
							sched_yield();
						}
					} else {
		...
		if (sitl_cfg.verbose && now - verbose_last_ns >= 1000000000ULL) {
			...
			if (gate_stall_count > 0) {
				fprintf(stderr, "SITL: gate stalls=%u stalled=%.1fms max_gap=%.1fus\n", gate_stall_count,
					gate_stall_wall_ns * 1e-6, fw_max_gap_ns * 1e-3);
			}
		}
	}
	return NULL;
...
void sitl_start_sim_thread(void)
{
	...
	DuplicateHandle(GetCurrentProcess(), GetCurrentThread(), GetCurrentProcess(), &fw_win_handle, 0, FALSE, DUPLICATE_SAME_ACCESS);
#else
