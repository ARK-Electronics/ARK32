...
usage: run_upstream_tests.py [--sitl path/to/elf]
exits non-zero if any test fails.
'''

# Adapted from am32-firmware/AM32 Mcu/SITL/run_ci_tests.py at
# 79af09b00c52021895cddff4080a983aa1b1279c. See UPSTREAM.md for local deltas.
import argparse
import glob
import os
import signal
...
def optional_unavailable(name, detail):
    if os.environ.get('SITL_REQUIRE_CAN') == '1':
        check(name + ' prerequisite', False, detail)
    else:
...
def can_state_stream(name):
    ...
    while time.time() < deadline and not sim.samples:
        time.sleep(0.2)
    if not sim.samples:
        optional_unavailable(name, 'SITL state stream never started with CAN enabled, '
...
def test_dronecan(sitl_path):
    try:
        ...
        optional_unavailable('dronecan', 'package not installed')
        return
...
def test_dronecan_params(sitl_path):
    ...
    try:
        ...
        optional_unavailable('dronecan params', 'package not installed')
        return
...
def test_dataset_params():
    ...
    # Keep ARK calibration fixtures and also validate upstream reference pairs.
    reference = [
        ('SEQURE_G431', 'sequre_gt2215'),
        ('VIMDRONES_L431', 'vimdrones_1404'),
        ('VIMDRONES_NANO_2216', 'vimdrones_nano_2216'),
    ]
    datasets = list(DATASETS) + [
        {'name': name, 'data': os.path.join(HERE, 'data', name),
         'model': os.path.join(HERE, 'models', model + '.json')}
...
def main():
    ...
    # A child can exit under a sanitizer before a protocol assertion fires.
    # Do not hide that report behind process teardown or optional CAN skips.
    log = open('sitl_ci.log', errors='replace').read() if os.path.exists('sitl_ci.log') else ''
    check('sanitizer log', not any(marker in log for marker in
          ('ERROR: AddressSanitizer', 'ERROR: LeakSanitizer', 'runtime error:')), 'no sanitizer diagnostics')
    if failures:
