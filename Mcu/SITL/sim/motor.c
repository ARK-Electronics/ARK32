...
void motor_step(uint64_t now_ns, uint32_t dt_ns)
{
	...
	if (sitl_cfg.battery.capacitance > 0) {
		...
		if (m.vc <= vset) {
			...
			if (sitl_cfg.battery.sink_current_max > 0 && i_sink > sitl_cfg.battery.sink_current_max) {
	...
	while (t_rem > 0) {
		...
		for (int pass = 0; pass < 4; pass++) {
			...
			for (int p = 0; p < 3; p++) {
				if (m.diode_state[p] == 0 || fabs(m.i[p]) > 1e-9 || !conducting[p] || hi[p] || lo[p]) {
					continue;
				}
	...
	for (int p = 0; p < 3; p++) {
		...
		m.e_br += int_iabs[p] * vf * f_diode + int_i2[p] * rds * (1.0 - f_diode);
	}
	...
	const double sf = sitl_cfg.motor.static_friction + stuck * sitl_cfg.motor.stuck_torque;
	if (fabs(w) < 0.5) {
	...
	if (sitl_cfg.physics_log) {
		...
		if (now_ns >= next_log_ns) {
			...
			snprintf(line, sizeof(line),
				 ...
				 " \"e_mag\": %.5f, \"e_cap\": %.4f}\n",
				 now_ns * 1e-9, m.omega * 60.0 / TWO_PI, m.vbus, m.ibus, (unsigned)duty_cycle, m.e_em, m.e_cu, m.e_br,
				 m.e_bus, m.e_sink, m.e_drag, 0.5 * L_eff * (m.i[0] * m.i[0] + m.i[1] * m.i[1] + m.i[2] * m.i[2]),
				 sitl_cfg.battery.capacitance > 0 ? 0.5 * sitl_cfg.battery.capacitance * m.vc * m.vc : 0.0);
			plog_push(line);
		}
	}
...
void motor_print_state(uint64_t now_ns, float time_ratio)
{
	// solver diagnostics: rates since the previous print
	{
		...
		fprintf(stderr, "SITL diag: comp_edges=%u diode_on_i=%u on_v=%u off=%u sign_viol=%u ev_ovf=%u\n",
			m.diag.comp_edges - last.comp_edges, m.diag.diode_on_i - last.diode_on_i, m.diag.diode_on_v - last.diode_on_v,
			m.diag.diode_off - last.diode_off, m.diag.sign_viol - last.sign_viol, m.diag.ev_overflow - last.ev_overflow);
		last = m.diag;
	}
