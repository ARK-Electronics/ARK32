...
struct cfg_entry {
	const char *section;
	const char *key;
	enum { CFG_FLOAT, CFG_INT, CFG_U32, CFG_BOOL } type;
	void *ptr;
};
...
static bool load_json_ex(const char *path, bool runtime)
{
	...
	for (int i = 0; i < tokens[0].size; i++) {
		...
		const bool skip = runtime && strcmp(secstr, "motor") != 0 && strcmp(secstr, "battery") != 0 && strcmp(secstr, "esc") != 0;
...
static void config_sanitise(void)
{
	...
	// keep L - M positive
	clampf(&sitl_cfg.motor.mutual_inductance, -1.0f, sitl_cfg.motor.inductance * 0.9f, "motor.mutual_inductance");
