...
class GdNfaultMachine:
    ...
    def cmd_held_add(self) -> None:
        if not self.cmd_arm:
            self.cmd_arm = 1
            self.cmd_t0 = self.ms
            return
        dt = _u16(self.ms - self.cmd_t0)
        self.cmd_t0 = self.ms
        if dt > 0xFFFF - self.cmd_held_ms:
            self.cmd_held_ms = 0xFFFF
        else:
            self.cmd_held_ms += dt
        ...
        if self.state == GD_NF_WARN:
            ...
            if current_live:
                ...
                if not self.dead_arm:
                    ...
                    self.enter_dead()
                    return
            if not self.otw_confirmed and self.cmd_held_ms >= GD["GD_WARN_CEIL_MS"]:
                self.enter_dead()
            return
