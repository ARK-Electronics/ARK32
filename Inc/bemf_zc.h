...
/* Clear acceleration-trend and turn-on-grid (duty-slew) state (bemf_zc.c).
 * Call next to every zero_crosses = 0 so a desync/coast/reverse cannot leave
 * a stale EMA or duty history that would bias the next closed-loop run once
 * ZC_TREND_MIN_ZC re-arms. */
